...
Image::Image(double x, double y, double z, double xc, double yc, double zc,
...
// code courtesy
// http://stackoverflow.com/questions/30423590/generate-bmp-file-from-array-of-rgb-values
// and bitmap documentation
...
void Image::bitmap(int width, int height, Pixel **grid) {
    ...
    for (int y = bih.biHeight - 1; y >= 0; y--) { /*Scanline loop backwards*/
        for (int x = 0; x < bih.biWidth; x++) {   /*Column loop forwards*/
            ...
            unsigned char r =
                (char)(rr / (factor3 *
                             factor3)); /*255 * ((double)x / bih.biWidth);*/
            unsigned char g =
                (char)(gg / (factor3 *
                             factor3)); /*255 * ((double)y / bih.biHeight);*/
